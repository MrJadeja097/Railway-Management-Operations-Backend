...
import { ArgumentNilException, DbException } from 'src/common/exceptions';
import { RPCNotFoundException } from 'src/common/exceptions/not_found.exception';
...
export abstract class BaseReadOnlyRepo<
  ...
  T,
  TKey,
  TPageableFilter extends PageableFilter<any> = PageableFilter<T, TKey>,
  TFilter extends Filter<any> = Filter<T, TKey>,
> implements IReadOnlyRepo<T, TKey, TPageableFilter, TFilter>
{
  ...
  public async getAsync(pk: TKey): Promise<T> {
    try {
      ...
      if (!e) throw new RPCNotFoundException();
      if (this.isDeleted(e)) {
        return null as T;
      }
  ...
  public async pagedAsync(filterObj :TPageableFilter = {} as TPageableFilter): Promise<IPageable<T>> {
    try {
      const opts = this.createFilterOpts(filterObj);
      if (!opts) throw new RPCNotFoundException();
  ...
  protected modifyFindOption(
    findOpts: FindManyOptions<TEntity>,
    filterObj: TFilter | TPageableFilter,
  ): void {
  ...
  protected softDeleteFilterOptions(): Record<string, unknown> {
    return this.createPartialWhere(
      this.softDeleteColumnName as string,
      null,
